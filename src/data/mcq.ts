...
MCQ.push(
  ...
  },
  {
    ...
    options: [
      'Ingress es más rápido que LoadBalancer porque procesa las peticiones en el data plane del cluster sin salir a internet; el LoadBalancer añade un hop extra de red al pasar las peticiones por la infraestructura del proveedor cloud antes de devolverlas al cluster.',
      'Ingress se incluye gratis en cualquier distribución de Kubernetes sin necesidad de instalar controladores adicionales; el IngressController ya está integrado en el apiserver de Kubernetes y no requiere componentes externos como nginx-ingress-controller o Traefik.',
      ...
      'LoadBalancer de Kubernetes no soporta TLS nativo porque opera en capa 4 (TCP) y no puede inspeccionar el contenido del paquete; para terminar TLS hay que usar siempre Ingress o un service mesh como Istio que añade el sidecar proxy con capacidades TLS en cada pod.',
    ],
    correctIndex: 2,
...
MCQ.push(
  ...
  },
  {
    ...
    options: [
      'No están disponibles en Java estándar y requieren librerías externas como Bouncy Castle para poder usarlos en aplicaciones Spring Boot de forma correcta y conforme a los estándares de seguridad NIST actuales.',
      'Solo pueden hashear texto ASCII de menos de 72 caracteres, lo que excluye contraseñas largas con emojis o caracteres especiales de alfabetos no occidentales que son comunes en aplicaciones internacionales.',
      ...
      'No producen hashes únicos: con alta probabilidad generan el mismo hash para contraseñas distintas (colisiones), lo que permite que dos usuarios con contraseñas diferentes puedan autenticarse con la misma contraseña en la base de datos.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Es una variante mejorada de OTP por SMS que usa el hardware del dispositivo (TPM o Secure Enclave) para generar el código temporal de seis dígitos, haciendo que el código sea más difícil de interceptar que el enviado por SMS pero manteniendo la misma experiencia de usuario.',
      ...
      'Requiere SMS como canal de verificación secundaria para la recuperación de cuenta porque FIDO2 no tiene mecanismos nativos de recuperación sin un segundo factor; sin SMS configurado, perder el autenticador físico implica pérdida permanente de acceso a la cuenta.',
      'Es más débil que TOTP clásico (Google Authenticator, Authy) porque depende del hardware del dispositivo del usuario; si el dispositivo falla o es robado, el atacante tiene acceso completo a todos los sitios registrados con ese autenticador sin necesidad de conocer ninguna contraseña adicional.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Por el número de desarrolladores del equipo: cada bounded context debería tener entre 3 y 7 personas según la ley de Conway, de forma que la comunicación dentro del contexto sea fluida y la comunicación entre contextos esté mediada por interfaces bien definidas.',
      ...
      'Por el microservicio que implementa la funcionalidad: cada microservicio tiene exactamente un bounded context asociado, y los límites del bounded context se determinan por las restricciones de despliegue y escalabilidad del servicio, no por el modelo del dominio.',
      'Por la base de datos que usa cada servicio: los bounded contexts se delimitan por el esquema de la base de datos, de modo que dos servicios que comparten el mismo esquema pertenecen automáticamente al mismo bounded context, aunque implementen funcionalidades de negocio distintas.',
    ],
    correctIndex: 1,
  ...
  // ── msg ───────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Solo cuando se añade un nuevo topic al broker; al añadir un topic existente el consumer group mantiene sus asignaciones actuales sin ninguna redistribución de particiones, porque el grupo solo monitoriza los topics a los que está suscrito explícitamente.',
      ...
      'Cada vez que se reinicia el broker líder del cluster, porque el nuevo líder invalida todos los metadatos de asignación de particiones que los consumers tenían en caché y fuerza una renegociación completa de las asignaciones con todos los consumer groups activos.',
      'Solo cuando la latencia de procesamiento del consumer supera el `max.poll.interval.ms` configurado en el broker; si el consumer procesa dentro de ese intervalo nunca se produce un rebalanceo aunque el consumer falle o pierda la conexión con el broker coordinator.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Elimina todos los mensajes cuyo offset es inferior al `log.retention.ms` configurado, independientemente de la key; la compaction funciona exactamente igual que la retención por tiempo pero usando el offset en lugar del timestamp como criterio de eliminación.',
      ...
      'Comprime los mensajes del topic con el algoritmo gzip, lz4 o snappy según la configuración del broker para reducir el ancho de banda entre producers y consumers; la compresión se aplica batch por batch y es transparente para el código de la aplicación.',
      'Agrupa todos los mensajes que comparten la misma key en un único batch compactado para reducir el número de operaciones de red; el consumer recibe un único mensaje por key en lugar de el historial completo, lo que mejora el throughput de consumo en topics de alta cardinalidad.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son equivalentes en comportamiento: ambos miden el intervalo desde el inicio de la ejecución anterior, la única diferencia es el nombre que los desarrolladores usan según el contexto; en implementaciones de JDK modernas el comportamiento interno es idéntico.',
      ...
      '`scheduleWithFixedDelay` solo acepta `Callable<V>` como argumento porque necesita el valor de retorno para calcular dinámicamente el siguiente delay; `scheduleAtFixedRate` acepta `Runnable` porque su intervalo es fijo y no depende del resultado de la ejecución.',
      '`scheduleAtFixedRate` cancela automáticamente la siguiente ejecución programada si la ejecución actual supera el periodo configurado, enviando una señal de interrupción al thread que ejecuta la tarea para garantizar que el intervalo nunca sea menor que el periodo especificado.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Los Virtual Threads no tienen acceso a `ThreadLocal` porque son gestionados por la JVM y no tienen un identificador de thread estable que pueda usarse como clave del mapa interno; intentar usar `ThreadLocal` en un VT lanza `UnsupportedOperationException` en tiempo de ejecución.',
      ...
      'Los `ThreadLocal` en Virtual Threads generan condiciones de deadlock cuando dos VTs comparten el mismo carrier thread y ambos intentan escribir en el mismo `ThreadLocal` al mismo tiempo, porque el carrier no puede ejecutar ambas escrituras de forma atómica sin mecanismos de sincronización adicionales.',
      '`ThreadLocal` no es thread-safe cuando se usa con Virtual Threads porque la JVM no garantiza que el mismo VT sea ejecutado siempre por el mismo carrier thread, y los valores del `ThreadLocal` quedan asociados al carrier, no al VT, causando que distintas ejecuciones del mismo VT vean valores diferentes.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    question: '¿Qué problema resuelve Structured Concurrency (JEP 437, Java 21 preview)?',
    options: [
  ...
  // ── rest ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Un protocolo de autenticación para APIs REST que extiende OAuth 2.0 añadiendo capacidades de negociación de permisos dinámicos; permite que el servidor informe al cliente qué acciones puede realizar sobre un recurso sin necesidad de documentación externa.',
      ...
      'Un formato de serialización alternativo a JSON para respuestas REST que incluye metadatos de tipo y schema en cada respuesta, facilitando la validación en el cliente y reduciendo la necesidad de documentación Swagger/OpenAPI adicional.',
      'Una estrategia de caché para APIs REST con alto tráfico que utiliza hypermedia tags para identificar qué recursos pueden almacenarse en caché CDN y por cuánto tiempo, similar a los Cache-Control headers pero con mayor granularidad por estado de la aplicación.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'El versionado por URL no es compatible con los principios REST porque REST exige que todos los endpoints sean stateless, y añadir un número de versión en la URL introduce estado implícito de versión que rompe la independencia de las peticiones individuales.',
      ...
      'El versionado por cabecera con `Accept: application/vnd.api+json;version=1` no funciona correctamente con proxies HTTP ni CDNs porque los proxies solo cachean por URL y no pueden diferenciar versiones basadas en cabeceras personalizadas sin configuración adicional.',
      'El versionado por URL obliga a duplicar toda la lógica de negocio en cada versión porque los frameworks REST no permiten compartir controladores entre versiones; cada endpoint versionado debe implementarse completamente de nuevo en una nueva clase o módulo independiente.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Las CTEs son siempre más rápidas que las subqueries equivalentes porque el motor de base de datos las materializa automáticamente en una tabla temporal en memoria antes de ejecutar la query principal, evitando recalcular el resultado múltiples veces.',
      ...
      'Las CTEs garantizan que el resultado nunca se materializa (siempre son inline): el optimizador las trata como vistas que se expanden en el plan de ejecución, lo que siempre produce un plan más eficiente que una subquery equivalente en un `FROM` o `WHERE`.',
      'Solo las CTEs pueden usar window functions como `ROW_NUMBER()`, `RANK()` o `LAG()`; las subqueries en el `FROM` no pueden contener window functions porque el optimizador no puede calcular el particionado sin una tabla temporal nombrada como contexto.',
    ],
    correctIndex: 1,
  ...
  // ── redis ─────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'No hay diferencia en el modelo de entrega: ambos entregan mensajes a todos los suscriptores activos con la misma semántica at-most-once; la única diferencia técnica es que Streams usa una estructura de datos indexada mientras que Pub/Sub usa canales de memoria sin persistencia.',
      ...
      'Pub/Sub tiene mayor throughput y menor latencia que Streams en todos los escenarios de producción porque no tiene el overhead de escritura en disco ni de gestión de offsets; Streams está diseñado exclusivamente para casos de uso donde el historial de mensajes es necesario para auditoría.',
      'Streams solo puede configurarse en topologías Redis Cluster con al menos 3 nodos maestros, lo que lo hace inviable para entornos de desarrollo o staging; Pub/Sub funciona en cualquier configuración Redis incluyendo instancias standalone y sentinelas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Garantiza ACID completo al igual que PostgreSQL: si cualquier comando del bloque falla por error de tipo o violación de constraint, todos los comandos anteriores del mismo MULTI/EXEC se revierten automáticamente al estado previo al inicio de la transacción.',
      ...
      'Permite revertir todos los comandos ejecutados dentro del bloque usando `DISCARD` inmediatamente después de `EXEC`; `DISCARD` funciona como un ROLLBACK SQL que devuelve Redis al estado exacto previo al `MULTI` inicial.',
      'Solo funciona en instancias Redis standalone sin réplicas; en Redis Cluster los bloques MULTI/EXEC se limitan a comandos que afecten a claves del mismo hash slot, y se ejecutan en el nodo maestro responsable de ese slot sin garantías de atomicidad entre slots distintos.',
    ],
    correctIndex: 1,
  ...
  // ── test ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Los tests con Testcontainers son más rápidos que con H2 porque los contenedores Docker usan la memoria RAM directamente sin el overhead del garbage collector de Java, mientras que H2 en modo in-memory añade latencia por la gestión de objetos en el heap de la JVM.',
      ...
      'Testcontainers solo puede usarse con bases de datos relacionales (PostgreSQL, MySQL, Oracle); para bases de datos NoSQL como MongoDB, Redis o Cassandra hay que usar mocks embebidos porque Docker no garantiza consistencia suficiente para tests de integración con sistemas distribuidos.',
      'Testcontainers no requiere Docker instalado localmente porque usa una implementación de contenedores basada en JVM incluida en la librería; el runtime de contenedores se gestiona internamente sin depender de ninguna instalación previa en el sistema operativo del desarrollador.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Es una técnica de fuzzing que genera datos de test aleatorios modificando los parámetros de entrada de cada método para encontrar casos límite que los tests manuales no contemplan; el mutation score mide qué porcentaje de los datos fuzzeados provocan fallos detectables.',
      ...
      'Es una técnica de refactoring automatizado que modifica el código fuente de los tests para eliminar duplicación y mejorar la legibilidad; el mutation score indica qué porcentaje de los tests refactorizados mantienen el comportamiento original sin introducir regresiones.',
      'Genera automáticamente tests de integración que verifican la consistencia de los datos entre capas de la aplicación; modifica los datos en la capa de persistencia y verifica que las capas superiores (servicio, API) reflejan correctamente los cambios sin inconsistencias.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Nunca deberías usar un Event Bus sobre Observer porque introduce un intermediario que oculta el flujo de datos, dificulta el debugging al hacer implícita la relación entre emisor y receptor, y añade latencia innecesaria por la capa de indirección del bus.',
      ...
      'El Event Bus de Guava y Spring ApplicationEventPublisher son exclusivamente asíncronos y ejecutan los listeners en un thread pool separado; el Observer clásico es siempre síncrono y es la única opción cuando necesitas que los observers reaccionen en el mismo thread que el subject.',
      'Observer es el patrón apropiado para eventos de UI (botones, formularios) porque los observers son conocidos y pocos; Event Bus se reserva para eventos de dominio de negocio (pedido creado, pago procesado) donde el número de listeners puede crecer indefinidamente.',
    ],
    correctIndex: 1,
  ...
  // ── cors ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Son el mismo ataque con nombres distintos según el contexto: CORS es el término moderno para CSRF cuando afecta a APIs JSON, mientras que CSRF es el término legacy que se usaba cuando los ataques se limitaban a formularios HTML y peticiones simples sin cabeceras personalizadas.',
      ...
      'CSRF es la amenaza que afecta a las APIs REST y que CORS mitiga completamente: configurar cabeceras `Access-Control-Allow-Origin` correctas en el servidor protege contra todos los ataques de tipo Cross-Site Request Forgery porque el browser bloquea las peticiones de orígenes no autorizados.',
      'CORS solo aplica a peticiones que devuelven respuestas JSON porque el navegador solo aplica la política de same-origin cuando el Content-Type es application/json; CSRF solo afecta a formularios HTML con Content-Type application/x-www-form-urlencoded o multipart/form-data.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Aumentar `maximumPoolSize` al máximo que permita el servidor de base de datos es siempre la solución más rápida y efectiva; las conexiones de base de datos son ligeras y el servidor puede gestionar miles de ellas sin impacto en rendimiento, por lo que un pool grande siempre es mejor que uno pequeño.',
      ...
      'El pool exhaustion solo ocurre con cargas de más de 100 peticiones por segundo concurrentes; para aplicaciones con menor tráfico el pool siempre tiene conexiones disponibles independientemente del tamaño configurado, por lo que este error nunca aparece en entornos de staging.',
      'La solución recomendada por Hikari es deshabilitar completamente el pool y usar conexiones directas bajo demanda en aplicaciones modernas con Virtual Threads, porque los VTs pueden gestionar miles de conexiones concurrentes sin los límites artificiales que impone un pool de tamaño fijo.',
    ],
    correctIndex: 1,
  ...
  // ── ia ────────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Una técnica de fine-tuning supervisado que adapta los pesos del LLM a un dominio específico entrenando el modelo con ejemplos del dominio objetivo; a diferencia de RAG, no requiere una base de conocimiento externa ni infraestructura de búsqueda semántica en producción.',
      ...
      'Un patrón de arquitectura para desplegar modelos LLM en producción con alta disponibilidad: distribuye las peticiones entre múltiples replicas del modelo y recupera automáticamente de fallos, garantizando SLAs de latencia P99 incluso bajo carga elevada de inferencia.',
      'Una técnica de compresión de prompts que analiza el texto del contexto y elimina tokens redundantes antes de enviarlo al modelo, reduciendo el coste de inferencia hasta un 70% sin pérdida significativa de calidad en la respuesta generada por el LLM.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Un ataque análogo a la SQL injection adaptado para bases de datos vectoriales: el atacante inyecta embeddings maliciosos en la base de conocimiento de un sistema RAG para contaminar los resultados de búsqueda semántica y hacer que el LLM recupere y reproduzca información manipulada.',
      ...
      'Una técnica legítima de optimización de prompts que consiste en inyectar instrucciones de formato y estilo al principio y al final de cada prompt para guiar al modelo hacia respuestas más concisas y estructuradas, reduciendo el coste de tokens en producción entre un 20-40%.',
      'Es un riesgo teórico que los LLMs modernos como GPT-4 o Claude han mitigado completamente en sus versiones de producción mediante filtros de seguridad en el pipeline de inferencia; las versiones enterprise de estos modelos tienen certificaciones de seguridad que garantizan inmunidad total a este tipo de ataques.',
    ],
    correctIndex: 1,
  ...
  // ── java21 ────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Permiten herencia múltiple de clases, lo que facilita componer comportamiento de varios padres sin duplicar código en las jerarquías de DTOs.',
      ...
      'Son mutables por defecto y se integran con JPA directamente, permitiendo mapear entidades con `@Entity` sin ningún adaptador adicional.',
      'Eliminan la necesidad de interfaces en el modelo de dominio, ya que el compilador infiere automáticamente los contratos a partir de los componentes declarados.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Permiten que una clase sea instanciada sin constructor visible, usando un factory implícito generado por el compilador para ocultar detalles de creación.',
      ...
      'Eliminan la necesidad de `final` en todas las subclases y permiten jerarquías abiertas con un sistema de versiones que detecta cambios entre compilaciones incrementales.',
      'Son equivalentes a `abstract` pero con mejor performance en tiempo de ejecución porque el JIT puede devirtualizar las llamadas al conocer el conjunto cerrado de implementaciones.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Se pinnea al hacer cualquier operación de I/O bloqueante, incluyendo lectura de archivos y conexiones JDBC; se evita envolviendo todas esas operaciones en `CompletableFuture.supplyAsync()`.',
      ...
      'Se pinnea exclusivamente al llamar métodos nativos (JNI) desde librerías de terceros; no existe ninguna forma práctica de evitarlo salvo reescribir esas librerías en Java puro.',
      'El pinning es una feature deliberada de Loom que mejora la localidad de caché del carrier thread; la JVM lo activa automáticamente para operaciones con alta reutilización de datos en CPU.',
    ],
    correctIndex: 1,
    explanation: 'Los VTs están diseñados para bloquearse en I/O (el carrier thread se libera). El problema es `synchronized`: si un VT entra en un bloque synchronized, el carrier thread queda ocupado hasta que el VT sale, anulando el beneficio de escalabilidad. Solución: usar `java.util.concurrent.locks.ReentrantLock` en lugar de `synchronized`. JDK internamente fue migrando clases con `synchronized` (ej. `java.util.zip.Deflater`, partes de `java.security` y NIO) hacia `ReentrantLock` para evitar pinning. Monitoriza pinning con `-Djdk.tracePinnedThreads=full`.',
  },
  {
    ...
    options: [
      'No existe `flatMap` en switch expressions; sin embargo `map` sí aplica y transforma el valor retornado de cada `case` de forma encadenada antes de asignarlo.',
      'Son equivalentes en switch con guarded patterns pero distintos en Stream: en el switch el compilador elige la variante más eficiente según el tipo en tiempo de compilación.',
      ...
      '`flatMap` aplana el resultado de varios cases en uno solo, útil cuando cada rama del switch devuelve una colección y se quiere obtener un único stream de resultados.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Que todas las tareas del scope se ejecuten en el mismo Virtual Thread para garantizar orden y evitar context switches entre ellas durante el procesamiento concurrente.',
      ...
      'Que todas las tareas del scope comparten automáticamente el mismo contexto transaccional de JTA, haciendo rollback global si cualquiera de ellas lanza una excepción no controlada.',
      'Que las tareas del scope se ejecutan en orden secuencial con reintentos automáticos configurables mediante una política de backoff exponencial sin código adicional.',
    ],
    correctIndex: 1,
    explanation: '`StructuredTaskScope` (JEP 437 en Java 21 preview; JEP 453 en Java 22) garantiza que el hilo que abre el scope espera a que todas las subtareas terminen antes de cerrar (try-with-resources). `ShutdownOnFailure`: si cualquier tarea falla, cancela el resto y relanza la excepción. `ShutdownOnSuccess`: en cuanto una tarea tiene éxito, cancela las demás. Esto da concurrencia estructurada con manejo de errores determinista.',
  },
  {
    id: 'mcq-java21-6', topicId: 'java21',
    question: '¿Qué colecciones añade `SequencedCollection` (Java 21) y qué métodos clave expone?',
    options: [
      'Añade `PriorityQueue` con acceso O(1) al máximo y mínimo, y expone `peek()`, `poll()` y `offer()` de forma consistente con el resto de la jerarquía de colecciones de Java.',
      ...
      'Reemplaza `ArrayList` con una implementación de acceso secuencial optimizada que reduce la fragmentación de memoria en inserciones al inicio de la lista.',
      'Añade operaciones de zip, unzip y sliding window a todas las colecciones existentes, siguiendo el modelo de Kotlin `Iterable` para mejorar la interoperabilidad entre bibliotecas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Siempre: los Virtual Threads son superiores en todos los escenarios; WebFlux/Reactor es código legacy que debe reemplazarse progresivamente en cualquier proyecto moderno con Java 21.',
      ...
      'WebFlux siempre es mejor porque usa menos memoria por solicitud y su model de event loop hace irrelevantes los Virtual Threads en cualquier aplicación Spring moderna.',
      'Virtual Threads solo funcionan correctamente con bases de datos NoSQL; para JDBC con PostgreSQL o MySQL siguen siendo necesarios los thread pools tradicionales o los drivers reactivos R2DBC.',
    ],
    correctIndex: 1,
  ...
  // ── webflux ───────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Porque solo puede emitir elementos nulos o vacíos hasta que alguien invoca `hot()` explícitamente para convertirlo en un stream con valores reales y emisión continua.',
      ...
      'Porque todas sus operaciones intermedias se ejecutan automáticamente en el scheduler `boundedElastic`, que está reservado para fuentes frías de datos como llamadas a BBDD o ficheros.',
      'Porque no puede emitir señales de error (`onError`), solo completar con `onComplete`, y esto lo hace seguro para pipelines de transformación de datos que no deben interrumpirse.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      '`flatMap` sirve exclusivamente para aplanar listas anidadas (`List<List<T>>`) dentro del stream; para transformaciones sobre tipos simples siempre debe usarse `map` independientemente de si la operación es síncrona o asíncrona.',
      ...
      'Son completamente intercambiables; la diferencia es únicamente de estilo y convención del equipo, aunque `flatMap` se considera más moderno en versiones recientes de Reactor.',
      '`flatMap` solo puede usarse con `Mono` y su firma no está disponible en `Flux`; para streams de múltiples elementos la alternativa correcta es `concatMap` que ofrece la misma semántica de aplanado.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son sinónimos funcionales: ambos cambian el scheduler de todo el pipeline completo, desde la fuente hasta el subscriber, y se comportan de forma idéntica independientemente de su posición en la cadena de operadores.',
      '`subscribeOn` determina en qué scheduler se ejecuta la suscripción (la señal subscribe recorre toda la cadena hasta la fuente); afecta a la fuente y todos los operadores hasta el primer `publishOn`, independientemente de dónde se coloque. `publishOn` cambia el scheduler para los operadores DOWNSTREAM a partir de ese punto.',
      '`publishOn` solo puede usarse con `Flux` para distribuir la emisión entre threads; `subscribeOn` es exclusivo de `Mono` porque solo hay un elemento y se puede controlar el thread de suscripción de forma determinista.',
      '`publishOn` está diseñado para operaciones de I/O no bloqueante y usa `Schedulers.parallel()` implícitamente; `subscribeOn` está orientado a operaciones CPU-intensive y siempre usa `Schedulers.boundedElastic()` como scheduler por defecto.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son funcionalmente equivalentes y solo difieren en el nombre: ambos aceptan una función que recibe el error y devuelven un nuevo Publisher alternativo; la elección entre uno u otro es puramente de legibilidad.',
      ...
      '`onErrorResume` solo captura `RuntimeException` y sus subclases; para `onErrorReturn` no hay restricción de tipo, lo que lo hace más adecuado para gestión de errores checked en pipelines de datos críticos.',
      '`onErrorReturn` reintenta la operación original hasta N veces antes de emitir el valor de fallback; `onErrorResume` no reintenta sino que sustituye directamente el publisher fallido por el alternativo.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'R2DBC delega todas las queries JDBC a un `ThreadPoolExecutor` dedicado con `boundedElastic()`, de modo que el event loop de Netty nunca toca el socket de la base de datos y los threads del pool absorben el tiempo de espera.',
      ...
      'R2DBC solo es compatible con bases de datos in-memory como H2 en modo reactivo, mientras que para PostgreSQL o MySQL sigue siendo necesario un adaptador bloqueante o el uso de `subscribeOn(boundedElastic())`.',
      'R2DBC precarga en caché todos los resultados posibles de la query antes de entregarlos al suscriptor, evitando así las esperas de I/O al precio de un mayor consumo de memoria inicial.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Es un mock completo del event loop de Netty que permite simular peticiones HTTP en pruebas de integración sin necesitar un servidor real, devolviendo respuestas predefinidas del pipeline reactivo.',
      ...
      'Solo verifica que el Publisher complete sin emitir señales de error, comprobando que el pipeline termina limpiamente; para verificar valores concretos hay que usar `blockFirst()` o `blockLast()`.',
      'Es una utilidad equivalente a `assertThat(flux.blockFirst())` con mejor stack trace, pero no soporta pipelines con retrasos temporales ni backpressure; para esos casos hay que usar `TestSubscriber` directamente.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'WebClient es más sencillo de configurar que RestTemplate porque no requiere definir un `MessageConverter` ni gestionar el `ObjectMapper`; Spring Boot lo autoconfigura completamente con soporte para JSON, XML y multipart de forma inmediata.',
      ...
      'WebClient tiene mejor soporte para autenticación básica y digest, ya que incluye filtros de seguridad preconfigurados que `RestTemplate` requiere configurar manualmente con `HttpComponentsClientHttpRequestFactory`.',
      'RestTemplate no puede deserializar respuestas JSON con tipos genéricos complejos; WebClient sí lo hace de forma nativa a través de `BodyExtractors` que resuelven el borrado de tipos en tiempo de ejecución.',
    ],
    correctIndex: 1,
  ...
  // ── react ─────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Siempre debe declararse con un array de dependencias vacío `[]` para que el efecto solo se ejecute al montar y desmontar el componente, evitando ejecuciones repetidas que puedan causar bucles infinitos de renderizado.',
      ...
      '`useEffect` nunca debe llamar funciones asíncronas directamente porque React no puede cancelar Promises; toda lógica asíncrona debe extraerse a un hook personalizado que gestione internamente la suscripción y la limpieza.',
      'Debe devolver siempre una función de cleanup aunque esté vacía para garantizar que React pueda gestionar correctamente el ciclo de vida del efecto en el modo estricto de desarrollo donde cada efecto se monta y desmonta dos veces.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Siempre que el componente use JSX con elementos interactivos o estáticos, ya que el compilador de Next.js necesita la directiva para saber que el componente debe incluirse en el bundle enviado al navegador.',
      ...
      'Cuando el componente hace fetch de datos desde una API externa, porque las llamadas de red solo pueden realizarse con seguridad desde el cliente para evitar problemas de CORS y no exponer credenciales en el servidor.',
      'Cuando el componente está dentro de la carpeta `app/` de Next.js, ya que todos los componentes dentro de esa carpeta son Client Components y necesitan la directiva para activar correctamente el runtime de React en el navegador.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    question: '¿Cuándo usar `providedIn: "root"` vs `providers: [SomeService]` en el `@Component` en Angular DI?',
    options: [
      '`providedIn: "root"` crea un singleton global (tree-shakeable). Añadir el servicio al array `providers: [SomeService]` del `@Component` crea una instancia nueva por componente, destruida con él. Útil para servicios con estado local. No existe el valor `"component"` en `providedIn`.',
      'Son equivalentes en Angular 17+: el compilador unifica ambas formas en una única instancia por módulo ngModule, ignorando si el servicio se declara en `providedIn` o en `providers` del decorador.',
      '`providers: [SomeService]` en el `@Component` es solo para directivas y pipes; para servicios de clase hay que usar exclusivamente la propiedad `providedIn` del decorador `@Injectable` con los valores `"root"`, `"platform"` o `"any"`.',
      '`providedIn: "root"` crea el servicio inmediatamente al arrancar la app (eager), aunque ningún componente lo haya inyectado aún, porque el compilador de Angular lo registra en el inyector raíz durante el bootstrap antes de renderizar el primer componente.',
    ],
    correctIndex: 0,
    explanation: '`providedIn: "root"`: tree-shakeable singleton (solo se incluye si se inyecta en algún lugar). Los valores válidos de `providedIn` son: `"root"`, `"platform"`, `"any"` — NO existe `"component"`. Para scope de componente: `providers: [SomeService]` en el `@Component` → instancia propia, destroyed con el componente. Ejemplo: `FormService` con estado del formulario → `providers` en el componente. `AuthService` global → `providedIn: "root"`. Los Standalone Components con `providers: []` permiten scope intermedio sin módulos. `inject()` (Angular 14+) como alternativa al constructor DI.',
  },
  ...
  // ── flink ─────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Event Time es más rápido porque delega el timestamp al productor Kafka, eliminando el overhead de los watermarks y reduciendo la latencia de procesamiento en ventanas cortas.',
      ...
      'Son equivalentes en la práctica: ambos usan el mismo reloj interno de Flink, pero Event Time añade un campo adicional de metadatos que los TaskManagers propagan entre operadores para sincronización.',
      'Processing Time se usa solo para fuentes Kafka porque lee el campo `timestamp` del mensaje; Event Time se usa con fuentes de fichero donde el orden de llegada coincide exactamente con el orden temporal real de los eventos.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Es una métrica del dashboard de Flink que muestra el lag acumulado entre el timestamp del último evento procesado y el reloj del sistema; se usa para alertas de SLA pero no afecta la lógica de ventanas.',
      ...
      'Es un mecanismo de checkpoint distribuido: el JobManager inserta watermarks entre barriers para marcar qué parte del state ya ha sido persistida, permitiendo recovery granular sin reprocessar todo el estado.',
      'Es una política de backpressure que limita cuántos eventos puede bufferizar un operador antes de bloquear al upstream, evitando OOM en operadores lentos con ventanas largas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son equivalentes en implementación; Sliding Window es el nombre que usa Flink internamente mientras que Tumbling Window es el nombre de la API pública para el mismo mecanismo de partición temporal.',
      ...
      'Tumbling Window solo se puede usar con Event Time porque requiere timestamps para calcular los límites fijos; Sliding Window funciona con los tres modos (Event, Ingestion, Processing) porque sus límites se calculan en relación al elemento actual.',
      'Sliding Windows almacenan solo las agregaciones parciales de cada slide, por lo que siempre consumen menos memoria que las Tumbling Windows, que deben mantener todos los eventos del periodo completo en buffer hasta que la ventana cierra.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son equivalentes: ambos permiten cualquier número de eventos intermedios entre los patrones A y B, pero `next()` usa contiguidad basada en offsets de Kafka mientras que `followedBy()` usa timestamps de Event Time para determinar la secuencia.',
      ...
      '`next()` aplica solo a secuencias dentro de la misma key del `keyBy()`; `followedBy()` permite detectar patrones cross-key, es decir, entre eventos de usuarios distintos en una misma ventana temporal del CEP.',
      '`followedBy()` solo se puede usar en el primer elemento del patrón CEP; en posiciones intermedias o finales del patrón es obligatorio usar `next()` para garantizar la determinismo del matching en streams de alta cardinalidad.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son registros de auditoría que el JobManager escribe en HDFS cada vez que un operador falla, almacenando el mensaje de error y el stack trace completo para facilitar el diagnóstico posterior sin necesidad de acceder a los logs distribuidos de los TaskManagers.',
      ...
      'Son puntos de sincronización que el programador declara explícitamente en el código del job con `env.checkpoint()`, definiendo qué partes del estado deben persistirse y cuáles pueden descartarse en caso de recovery para reducir el tamaño del snapshot.',
      'Son equivalentes a los savepoints en funcionalidad, pero se diferencian en que los checkpoints los gestiona el operator graph de forma automática, mientras que los savepoints requieren intervención manual del operador del cluster para dispararse y almacenarse.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    question: '¿Cuándo usar `RocksDBStateBackend` en lugar de `HashMapStateBackend` (anteriormente FsStateBackend en Flink <1.13)?',
    options: [
      'RocksDB debe usarse siempre como backend por defecto porque su implementación en C++ es más eficiente que el HashMap de Java para cualquier tamaño de estado, y la diferencia de latencia de acceso al disco se amortiza completamente por el block cache interno de RocksDB.',
      ...
      'RocksDB solo puede usarse en jobs de streaming puro con Event Time; para jobs de batch o streaming con Processing Time la serialización de RocksDB genera inconsistencias en el ordering que rompen la semántica exactly-once del checkpoint.',
      'RocksDB es el único backend compatible con el Flink Kubernetes Operator porque requiere PersistentVolumeClaims para el almacenamiento local; HashMapStateBackend solo funciona en despliegues standalone con almacenamiento efímero.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      '`keyBy()` es una operación opcional de optimización que hints al scheduler de Flink para colocar subtasks relacionados en el mismo TaskManager, reduciendo el tráfico de red entre operadores cuando el estado es grande y los eventos de la misma clave se procesan frecuentemente juntos.',
      ...
      '`keyBy()` es el mecanismo de Flink para hacer joins entre dos streams: define la clave de unión igual que el campo ON de SQL, y después del `keyBy()` se puede usar `connect()` y `CoProcessFunction` para acceder al estado de ambos streams en el mismo operador.',
      '`keyBy()` convierte el `DataStream<T>` en un `WindowedStream` virtual que acumula eventos en memoria hasta que se aplica una función de ventana, momento en que Flink los procesa como un micro-batch y descarga el buffer para liberar memoria del TaskManager.',
    ],
    correctIndex: 1,
  ...
  // ── cicd ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Es un log append-only de cada operación `plan` y `apply` ejecutada; borrarlo solo elimina el historial de cambios, pero Terraform puede reconstruirlo automáticamente consultando el provider en el siguiente `plan` para redescubrir todos los recursos existentes.',
      ...
      'Es el equivalente a un Dockerfile pero para infraestructura: define la imagen base del entorno y las instrucciones de construcción que Terraform ejecuta de forma reproducible en cualquier CI/CD sin necesidad de credenciales adicionales.',
      'Se almacena remotamente únicamente para mejorar la velocidad de `terraform plan`, porque el provider API tiene rate limiting y el state local obliga a consultar todos los recursos en cada ejecución, mientras que el remoto cachea las respuestas durante 24 horas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Ansible ejecuta siempre los mismos comandos shell en cada ejecución del playbook para garantizar consistencia; la idempotencia se logra añadiendo `ignore_errors: yes` a todas las tasks para que no fallen si el recurso ya existe en el estado deseado.',
      ...
      'Idempotencia en Ansible significa que el playbook está pensado para ejecutarse una única vez durante el ciclo de vida del servidor; ejecutarlo dos veces generaría configuraciones duplicadas o conflictos de versión en paquetes como nginx o python.',
      'Solo los módulos de los proveedores cloud (aws_ec2, azure_vm, gcp_compute) garantizan idempotencia completa porque los proveedores cloud los diseñaron con APIs declarativas; los módulos de sistema operativo como `apt` o `yum` son imperativos y no idempotentes por defecto.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'No hay diferencia en la ejecución; `parallel` es únicamente un bloque de agrupación visual en la interfaz de Jenkins Blue Ocean que permite colapsar varios stages relacionados en una sola fila del pipeline graph sin cambiar el orden secuencial de ejecución.',
      ...
      '`parallel` solo está disponible en pipelines scripted (con `node {}`) y no puede usarse en la sintaxis declarativa (con `pipeline {}`); en pipelines declarativos la concurrencia se consigue con `options { throttleJobProperty() }` y múltiples ejecutores.',
      '`parallel` requiere que cada rama se ejecute en un agente Jenkins distinto con label específico; en agents compartidos o en el agente master, Jenkins serializa automáticamente las ramas paralelas para evitar condiciones de carrera en el workspace.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Es una regla de firewall en el servidor SonarQube que bloquea análisis provenientes de ramas feature para proteger la calidad del análisis de la rama principal, permitiendo solo análisis de ramas main o release configuradas en el repositorio.',
      ...
      'Es una credencial de autenticación que SonarQube exige al scanner de Jenkins para poder subir los resultados del análisis; sin este token los resultados se almacenan solo localmente en el workspace del agente y no aparecen en el dashboard.',
      'Solo aplica a proyectos Java analizados con Maven o Gradle; para proyectos JavaScript, Python o C# se usan los External Issues Reporters que tienen su propio sistema de umbrales separado de los Quality Gates convencionales.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'GitOps es una metodología de branching que extiende GitFlow añadiendo ramas `ops/` para cambios de infraestructura y `config/` para variables de entorno; ArgoCD monitoriza estas ramas específicas y aplica los cambios automáticamente al cluster cuando pasan los checks de PR.',
      ...
      'ArgoCD funciona exclusivamente con webhooks: cada repositorio Git debe configurar un webhook hacia el servidor ArgoCD para notificarle los cambios; sin webhook configurado ArgoCD no puede detectar nuevos commits y el sync debe dispararse manualmente desde la UI.',
      'GitOps aplica únicamente a aplicaciones stateless porque ArgoCD no puede gestionar el estado de bases de datos ni Persistent Volumes; para componentes stateful se recomienda combinar ArgoCD con Helm Secrets y operadores de base de datos gestionados manualmente.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Debe usarse habitualmente para acelerar los ciclos de desarrollo: al limitarse a un recurso, el plan es mucho más rápido porque no consulta todos los providers y el state remoto completo; es la alternativa recomendada a los módulos separados para equipos con monorepos de infra grandes.',
      ...
      '`-target` es equivalente a `-replace` porque ambos recrían el recurso desde cero ignorando el state actual; la diferencia es que `-replace` fuerza la recreación mientras que `-target` reutiliza el recurso existente si ya está en el state sin destruirlo primero.',
      '`-target` solo acepta módulos completos como argumento (`-target=module.red`); para recursos individuales hay que usar el flag `--resource-address` con la sintaxis completa del tipo y nombre del recurso en el HCL del provider correspondiente.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son la configuración de privilegios de escalada en Ansible: definen si las tasks se ejecutan como root (`become: yes`) o como el usuario SSH de conexión, permitiendo separar tasks privilegiadas de las no privilegiadas dentro del mismo playbook sin cambiar el usuario de conexión.',
      ...
      'Son módulos Ansible personalizados escritos en Python que encapsulan lógica de negocio compleja no disponible en los módulos oficiales; se distribuyen como ficheros `.py` en la carpeta `library/` del playbook y se invocan igual que los módulos estándar de Ansible.',
      'Las Roles solo pueden usarse con inventarios dinámicos que consulten proveedores cloud (AWS EC2, Azure ARM, GCP Compute) porque la resolución de las dependencias entre roles requiere conocer la topología real de los servidores en el momento de la ejecución del playbook.',
    ],
    correctIndex: 1,
  ...
  // ── python ────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Ambas formas devuelven un DataFrame de una sola columna; la diferencia es que el doble corchete crea una vista read-only mientras que el corchete simple devuelve una copia mutable, lo que afecta si las modificaciones posteriores se propagan al DataFrame original.',
      'La versión con doble corchete siempre realiza una copia profunda del DataFrame completo antes de filtrar la columna, lo que garantiza que las operaciones posteriores no afectan al original; el corchete simple devuelve siempre una vista y modificarla puede generar el SettingWithCopyWarning.',
      ...
      'La versión con doble corchete es la sintaxis legacy de Pandas 0.x que mantiene compatibilidad hacia atrás; en Pandas 2.0+ se deprecó a favor de `df["col"]` porque la API de selección se unificó para devolver siempre Series independientemente del número de corchetes.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      ...
      'Pydantic v2 añade soporte nativo para validación asíncrona con `async_validate()`, lo que permite validar modelos que dependen de datos de BD o APIs externas sin bloquear el event loop de FastAPI ni necesitar librerías adicionales.',
      'La principal mejora de Pydantic v2 es la integración nativa con SQLAlchemy 2.0: los modelos Pydantic se pueden usar directamente como declaraciones ORM eliminando la necesidad de DTOs separados y la capa de conversión entre modelos de dominio y modelos de API.',
      'Pydantic v2 introduce breaking changes que lo hacen incompatible con cualquier versión de FastAPI anterior a 0.100; además requiere Python 3.11+ por las optimizaciones del intérprete que usa el core de Rust para serialización de tipos complejos.',
    ],
    correctIndex: 0,
  ...
  },
  {
    ...
    options: [
      'Solo cuando el DataFrame tiene más de 10 columnas numéricas, porque las llamadas separadas son más eficientes para DataFrames pequeños al evitar la creación de los objetos intermedios que `agg()` necesita para gestionar el diccionario de funciones.',
      ...
      'Exclusivamente cuando necesitas usar lambdas personalizadas, ya que `sum()` y `mean()` son funciones builtin de Pandas que no aceptan parámetros adicionales; `agg()` es el único método que permite pasar callables y funciones de NumPy como argumento de agregación.',
      '`agg()` y las llamadas separadas son completamente equivalentes en rendimiento y funcionalidad; la única diferencia es que `agg()` devuelve un DataFrame con MultiIndex en las columnas cuando se aplican múltiples funciones, lo que puede complicar el acceso a los resultados.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Convierte la dependencia en un generador que puede retornar múltiples valores durante la vida del request: FastAPI llama a `next()` cada vez que el handler necesita un nuevo valor, permitiendo streaming de datos desde la dependencia al handler de forma incremental.',
      'Indica que la función de dependencia es asíncrona e internamente usa `await`; FastAPI lo detecta automáticamente y la ejecuta en el event loop en lugar del thread pool, optimizando el rendimiento para dependencias que hacen I/O como consultas a base de datos.',
      ...
      'Es un mecanismo de caché por cliente: FastAPI mantiene el valor del yield en sesión por IP o token JWT durante la vida del proceso, reutilizándolo en requests posteriores del mismo cliente para evitar reconexiones innecesarias a la base de datos.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Cachea la respuesta completa del modelo cuando la petición es idéntica a una anterior, devolviendo el mismo output sin consumir tokens adicionales; útil para endpoints de FAQ donde las preguntas de los usuarios se repiten frecuentemente con los mismos términos exactos.',
      'Aplica solo al primer bloque del historial de mensajes de usuario, ignorando el system prompt y los tool definitions; las conversaciones con historial largo no se benefician porque los mensajes anteriores siempre se reprocesar en cada turno de la conversación.',
      'Es un caché transparente gestionado por la infraestructura CDN de Anthropic que almacena las activaciones intermedias del modelo para prefijos frecuentes; no requiere ninguna configuración por parte del desarrollador y se activa automáticamente para todos los modelos.',
    ...
    correctIndex: 3,
    explanation: 'El prompt caching de Anthropic funciona a nivel de prefijo: si envías el mismo system prompt largo en múltiples llamadas, el modelo solo procesa los tokens de caché la primera vez. Los tokens de "cache read" cuestan ~0.30$/MTok (≈ 10% del precio normal de entrada de ~3$/MTok en claude-sonnet-4-5), es decir, un ahorro del ~90%. Ver `usage.cache_read_input_tokens` en la respuesta para saber si el caché se usó. Muy útil para chatbots con system prompts largos o RAG con documentos de contexto.',
  },
  {
    id: 'mcq-python-6', topicId: 'python',
    question: '¿Qué son los type hints en Python y cómo los usa Pydantic para validación?',
    options: [
      'Los type hints son anotaciones que el intérprete de Python evalúa en runtime desde Python 3.10+; si el valor no coincide con el tipo anotado, Python lanza un `TypeError` automáticamente sin necesidad de librerías adicionales como Pydantic o mypy.',
      ...
      'Los type hints en Python son obligatorios desde la versión 3.12 para todos los parámetros de función y atributos de clase porque el recolector de basura usa la información de tipo para optimizar la gestión de memoria de objetos con ciclos de referencia.',
      'Pydantic lee los type hints únicamente para generar el JSON Schema que FastAPI expone en `/docs` (OpenAPI); la validación real de los datos de entrada la realiza el servidor ASGI (Uvicorn/Gunicorn) antes de que el request llegue al handler de FastAPI.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      '`merge` y `join` son exactamente equivalentes internamente; `join` es un método de instancia que delega en `pd.merge()` con los mismos parámetros, y la única diferencia práctica es que `join` tiene un nombre más corto de escribir en cadenas de métodos fluidas.',
      ...
      '`merge` solo puede unir DataFrames con el mismo número de filas porque realiza un producto cartesiano interno y luego filtra; `join` usa un algoritmo hash más eficiente que soporta DataFrames de tamaños muy distintos sin penalización de memoria.',
      '`join` soporta los cuatro tipos de unión (inner, outer, left, right) al igual que SQL, mientras que `merge` solo implementa inner join y cross join; para outer joins hay que usar `concat()` con `axis=1` y gestionar los NaN resultantes manualmente.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Con Erasure Coding EC 4+2, el multiplicador de almacenamiento es 1.5× (50% de overhead extra), frente a 3× (200% extra) de replicación ×3. Ambos toleran 2 fallos simultáneos, pero EC ahorra espacio a cambio de mayor CPU en writes y reconstrucciones.',
    ...
    correctIndex: 0,
    explanation: 'Replicación ×3: 3 copias idénticas → 3× los datos originales (2TB de overhead por cada TB almacenado). Erasure Coding EC 4+2: se divide el objeto en 4 fragmentos de datos + 2 de paridad → 1.5× los datos originales (0.5TB de overhead por cada TB). Ambos toleran 2 fallos, pero EC lo hace con mucho menos espacio desperdiciado. Trade-off: EC requiere más CPU para computar los fragmentos de paridad en cada write y para reconstruir en lecturas con fallos. Para datos fríos/backup, EC es ideal. Para alta frecuencia de pequeños writes, replicación puede ser más rápida.',
  },
  {
  ...
  // ── redux ─────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Escribir reducers asíncronos directamente con async/await: Immer detecta las promesas devueltas por el reducer y espera su resolución antes de producir el nuevo estado, eliminando la necesidad de `createAsyncThunk`.',
      ...
      'Compartir el mismo objeto de estado entre múltiples stores de Redux simultáneamente, de modo que un cambio en un store se propaga automáticamente a todos los demás stores que usan el mismo slice sin necesidad de sincronización manual.',
      'Eliminar completamente la necesidad de action creators y types separados: Immer analiza las mutaciones del reducer y genera automáticamente los action types correspondientes basándose en los nombres de las propiedades modificadas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Solo `users/fetch/fulfilled` cuando la función no lanza errores; si hay un error, el thunk propaga la excepción directamente al componente sin dispatch de ninguna action adicional, dejando al componente gestionar el error con un try/catch local.',
      '`users/fetch/pending`, `users/fetch/fulfilled` y `users/fetch/rejected` — una por cada fase del ciclo de vida asíncrono.',
      'Genera `users/fetchStart`, `users/fetchEnd` y `users/fetchError` siguiendo la convención de nombres de Redux tradicional; estas tres actions tienen que añadirse manualmente al `reducers` del slice porque `extraReducers` solo acepta los names generados por el CLI de Redux Toolkit.',
      'Solo genera la función thunk en sí; los tres action types (`pending`, `fulfilled`, `rejected`) tienen que definirse manualmente en el slice con `createAction()` y después registrarse en `extraReducers` para conectarlos con el estado de carga.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Porque delega la ejecución del selector a un Web Worker separado del hilo principal de React, liberando el event loop de los cálculos costosos y permitiendo que la UI siga respondiendo mientras se procesa el estado del store.',
      ...
      'Porque convierte los Observables del store en Promises resueltas síncronamente, lo que permite que React procese los cambios de estado en un único microtask en lugar de en múltiples macrotasks, reduciendo el número de repintados del DOM.',
      'Porque elimina la necesidad de usar `useSelector` en los componentes al conectar directamente el store con el árbol de React mediante Context, de forma similar a cómo funciona `react-redux` connect() en los componentes de clase.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Elimina definitivamente las entradas del cache con esos tags y las descarta de la memoria del QueryClient sin posibilidad de refetch; si un componente montado necesita esos datos tendrá que hacer una nueva query desde cero con `refetch()` explícito.',
      'Marca como "stale" las queries que provean esos tags, forzando un refetch automático la próxima vez que sean necesarias.',
      'Lanza un error de validación en tiempo de ejecución si los datos del servidor no coinciden con el schema TypeScript de la query, evitando que datos corruptos o mal formados lleguen a los componentes de la aplicación.',
      'Cancela inmediatamente todas las peticiones HTTP en vuelo que tengan esos tags, usando `AbortController` internamente para interrumpir las llamadas al servidor, de forma similar a como `switchMap` cancela Observables en RxJS.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Siempre que el proyecto sea nuevo: Context API es el sucesor oficial de Redux según los docs de React, que recomiendan migrar todos los proyectos existentes que usen Redux porque Context tiene mejor integración con las nuevas primitivas de React 18 como `useTransition` y `useDeferredValue`.',
      'Para estado global frecuentemente actualizado por muchos componentes: Redux escala mejor con selectores memoizados y evita los re-renders masivos que provoca Context al cambiar el valor.',
      'Para configuración estática o raramente cambiada (tema, idioma, locale): Context es suficiente y más sencillo.',
      'Context API está limitada a compartir estado solo entre componentes que tienen una relación directa padre-hijo en el árbol; no puede hacer que componentes en ramas completamente separadas del árbol compartan estado sin duplicar el Provider.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Serializa cada action a JSON antes de pasarla al reducer, verificando que los datos son serializables y compatibles con Redux DevTools; si la action contiene funciones, clases o referencias circulares, el middleware lanza un error de serialización para prevenir bugs difíciles de depurar.',
      ...
      'Valida en runtime que cada action despachada tenga exactamente las propiedades `type` y `payload` definidas en el schema del slice, bloqueando las actions malformadas antes de que lleguen al reducer y generando mensajes de error descriptivos en la consola.',
      'Conecta automáticamente el store de Redux con la extensión Redux DevTools del navegador, inyectando el enhancer necesario para el time-travel debugging y la inspección del historial de actions sin necesidad de configuración adicional.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Hace polling al store cada 16ms sincronizándose con el ciclo de animación del navegador mediante `requestAnimationFrame`, de modo que los cambios de estado solo se aplican una vez por frame para evitar re-renders excesivos en componentes con actualizaciones frecuentes.',
      ...
      'Solo re-renderiza el componente si está envuelto en `React.memo` con un comparador custom; sin `React.memo`, `useSelector` no tiene efecto y el componente siempre se re-renderiza cuando el store cambia, independientemente de si el valor del selector ha cambiado.',
      'Usa la API `MutationObserver` del navegador para detectar cambios en el DOM provocados por actualizaciones del store, re-ejecutando el selector solo cuando los nodos DOM relevantes para el componente han sido modificados por un render previo.',
    ],
    correctIndex: 1,
  ...
  // ── ngrx ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Para poder serializar el reducer junto con el estado en el fichero de configuración de Angular, permitiendo que el CLI regenere el store automáticamente sin perder el estado en hot module replacement durante el desarrollo.',
      'Porque NgRx ejecuta los reducers en un Web Worker separado del hilo de Angular para no bloquear la change detection durante el procesamiento de actions con payloads grandes o cálculos complejos de transformación de estado.',
      ...
      'Porque el compilador de Angular CLI analiza el código del reducer durante el build y solo puede aplicar tree-shaking y optimizaciones AOT si detecta que la función no tiene dependencias externas ni efectos secundarios visibles.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Son completamente intercambiables en la práctica; la convención de usar switchMap para búsquedas y mergeMap para cargas individuales es solo una preferencia de estilo del equipo de NgRx sin impacto funcional real en el comportamiento del store.',
      ...
      'switchMap solo puede usarse en Effects que tienen `dispatch: false` porque cancela las actions pendientes en el bus antes de que lleguen al reducer; mergeMap es obligatorio en Effects que dispatchen acciones para garantizar que todas las actions despachadas llegan al store en orden.',
      'mergeMap solo funciona correctamente con Observables fríos como las peticiones HTTP; con Observables calientes como WebSockets hay que usar switchMap porque mergeMap no puede gestionar múltiples suscriptores simultáneos en fuentes de datos compartidas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Signal Store solo puede gestionar estado síncrono porque los Signals de Angular son síncronos por definición; para cualquier operación asíncrona como peticiones HTTP hay que combinar Signal Store con el Store clásico o usar servicios con `async/await` separados del store.',
      ...
      'Signal Store solo funciona con Standalone Components porque usa el sistema de inyección funcional `inject()` que no es compatible con los módulos NgModule tradicionales de Angular; para migrar un módulo existente hay que convertirlo primero a Standalone.',
      'Signal Store y el Store clásico son idénticos en API y comportamiento; Signal Store es únicamente un alias más corto para la misma implementación interna, añadido en NgRx 17 por razones de marketing y para alinearse con la nomenclatura de Angular Signals.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'EntityAdapter almacena automáticamente las entidades en IndexedDB del navegador para persistencia offline, de modo que el estado sobrevive a recargas de página sin necesidad de configurar `localStorage` ni serialización manual del store de NgRx.',
      ...
      'EntityAdapter genera automáticamente los Selectors para todas las operaciones de acceso a la colección mediante análisis estático del tipo de entidad, eliminando la necesidad de usar `createSelector` y simplificando el testing al no haber selectores que mockear.',
      'EntityAdapter solo funciona con entidades que tienen un campo llamado exactamente `id` de tipo `number`; para entidades con claves de tipo `string` o con nombres de campo distintos como `uuid` o `key` hay que usar arrays normales con gestión manual.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Importando el `StoreModule.forRoot()` completo de NgRx en el `TestBed` con un initialState realista; esto garantiza que el store tiene el mismo comportamiento que en producción y evita los falsos negativos que puede introducir un mock store en pruebas de integración.',
      ...
      'Solo mediante pruebas E2E con Cypress o Playwright, ya que los componentes que inyectan el Store de NgRx en el constructor no pueden instanciarse de forma aislada en un TestBed sin que el módulo completo de NgRx esté disponible con todos sus reducers y effects registrados.',
      'Sustituyendo el Store inyectado por un objeto JavaScript plano que tenga métodos `dispatch()` y `select()` que devuelvan `of(fakeValue)`, sin necesitar `provideMockStore` ni importar ningún módulo de NgRx en el TestBed del test.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Crea nuevas instancias de las actions con el payload proporcionado por parámetro, permitiendo que el Effect genere actions de respuesta sin importar los action creators desde el módulo correspondiente; es el mecanismo de factory de actions dentro del pipeline de Effects.',
      ...
      'Valida en tiempo de ejecución que el payload de la action tiene la estructura correcta según los types definidos con `props<{}>()`, lanzando un `TypeError` descriptivo si el payload está incompleto o tiene tipos incorrectos antes de que llegue al pipeline del Effect.',
      'Convierte el Observable de actions en una Promise compatible con async/await, permitiendo usar `await this.actions$.pipe(ofType(loadUsers))` en functions asíncronas sin tener que manejar el patrón Observable en Effects que solo necesitan ejecutarse una vez.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'OnPush no es compatible con NgRx porque el store de NgRx usa ZoneJS internamente para notificar cambios de estado, y OnPush desactiva ZoneJS para ese componente; la combinación de ambos genera un conflicto que hace que los componentes no se actualicen cuando el estado del store cambia.',
      ...
      'NgRx detecta automáticamente durante el build qué componentes usan `store.select()` y aplica `ChangeDetectionStrategy.OnPush` a todos ellos sin necesidad de configuración manual, como parte de las optimizaciones que hace el `StoreModule.forRoot()` al registrarse.',
      'Con OnPush hay que llamar `store.dispatch()` dos veces consecutivas para cada cambio de estado porque la primera llamada actualiza el store internamente pero Angular necesita un segundo ciclo de digest para detectar el cambio de referencia en el `async` pipe del template.',
    ],
    correctIndex: 1,
  ...
  // ── rxjs ──────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Los Observables fríos son síncronos y emiten todos sus valores antes de pasar el control al siguiente operador del pipe; los calientes son asíncronos y usan el event loop para entregar cada valor en un microtask separado, lo que permite que otros Observables se intercalen.',
      ...
      'Los Observables fríos están limitados a un número finito de valores porque se completan automáticamente después de emitir; los calientes son necesariamente infinitos porque representan fuentes de eventos que nunca terminan como WebSockets o streams de DOM.',
      'Los Observables calientes requieren obligatoriamente un Subject como puente porque ningún Observable nativo de RxJS es hot; los fríos se crean con el constructor `new Observable()` o con `Observable.create()` y solo pueden ejecutarse de forma aislada por suscriptor.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Encola el nuevo valor en un buffer interno hasta que el Observable activo complete, y entonces lo procesa en orden; es equivalente a `concatMap` pero con el beneficio adicional de que el buffer tiene un límite configurable para evitar acumulación de memoria.',
      'Cancela (unsubscribe) el Observable interno activo y se suscribe al nuevo. Solo el último Observable interno puede emitir.',
      'Emite los valores de ambos Observables en paralelo sin cancelar ninguno, exactamente como `mergeMap`; la única diferencia entre `switchMap` y `mergeMap` es el orden en que aparecen los valores emitidos en el stream resultante cuando los Observables internos completan.',
      'Lanza un error de concurrencia detectable con `catchError` que incluye el valor del Observable cancelado en la propiedad `cancelledValue`, permitiendo gestionar los reintentos de las peticiones interrumpidas sin perder los datos originales.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son completamente equivalentes en comportamiento; la única diferencia es que `forkJoin` fue introducido en RxJS 5 y `combineLatest` en RxJS 6, por lo que en proyectos que usen RxJS 7+ ambos pueden usarse indistintamente sin ninguna diferencia funcional observable.',
      ...
      '`forkJoin` solo acepta arrays de Observables como argumento; `combineLatest` acepta exactamente dos Observables que se combinan por pares; para combinar más de dos fuentes hay que anidar múltiples `combineLatest` o convertirlos a array con el operador `merge` primero.',
      '`combineLatest` cancela automáticamente los Observables más lentos del grupo cuando el más rápido ha completado su emisión de valores, optimizando el rendimiento al no esperar por fuentes lentas que retrasarían el resultado final del operador.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Replay solo aplica al primer suscriptor que se conecta después de que el Observable haya emitido; los suscriptores posteriores al segundo no reciben valores cacheados y tienen que esperar a que el Observable emita nuevos valores desde el momento de su suscripción.',
      ...
      '`shareReplay(1)` es exactamente equivalente a `publishReplay(1).refCount()` en términos de comportamiento; ambos usan el mismo mecanismo de ReplaySubject internamente y comparten las mismas características de gestión de suscriptores y cancelación de upstream.',
      'Solo funciona correctamente con Observables síncronos que emiten todos sus valores antes del primer `tick` del event loop; con Observables asíncronos como HTTP o timers, el valor cacheado puede quedar desactualizado porque el replay no detecta cuándo el upstream ha emitido nuevos valores.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Limita el número máximo de valores que puede emitir el Observable a un umbral configurable (por defecto 1000), completando automáticamente la suscripción cuando se alcanza el límite para evitar que streams de larga duración acumulen memoria en los componentes.',
      ...
      'Registra la suscripción en un `WeakRef` en lugar de una referencia fuerte, permitiendo que el GC libere tanto el Observable como el componente de forma simultánea cuando no quedan referencias fuertes al componente en ninguna parte del árbol de Angular.',
      'Solo funciona correctamente si se coloca como primer operador en el pipe antes de cualquier transformación; si se coloca en otra posición, el operador no puede interceptar la señal de destrucción del componente y la suscripción permanece activa hasta que el Observable complete por sí mismo.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      '`delay(300)` seguido de `distinctUntilChanged()`: el delay introduce una pausa antes de emitir cada valor, y distinctUntilChanged filtra los duplicados consecutivos; juntos reproducen el comportamiento de debounce sin necesidad de operadores especializados.',
      '`throttleTime(300)`: limita las emisiones a una por cada ventana de 300ms, garantizando que las peticiones se espacian uniformemente independientemente del ritmo de escritura del usuario y sin perder ninguna consulta porque el throttle siempre emite el primer valor de cada ventana.',
      '`debounceTime(300)` + `distinctUntilChanged()` + `switchMap(query => search(query))`',
      '`auditTime(300)` seguido de `mergeMap(query => search(query))`: auditTime emite el último valor al final de cada ventana de 300ms sin importar cuándo llegó, y mergeMap ejecuta todas las peticiones en paralelo para que la más rápida llegue primero al suscriptor.',
    ],
    correctIndex: 2,
  ...
  },
  {
    ...
    options: [
      'Son equivalentes en efecto práctico: ambos previenen que el error llegue al suscriptor final; la diferencia es que `retry` hace el reintento de forma síncrona mientras que `catchError` lo hace de forma asíncrona en el siguiente tick del event loop.',
      ...
      '`retry` solo puede usarse con Observables que representan peticiones HTTP porque necesita que la fuente sea re-ejecutable desde el principio; `catchError` funciona con cualquier Observable incluyendo fuentes de eventos que no pueden repetirse como `fromEvent` o WebSockets.',
      '`catchError` suspende el stream temporalmente en el punto del error hasta que el operador decide qué hacer, manteniendo los valores buffereados para entregarlos cuando se retome; `retry` cancela definitivamente todos los valores pendientes y comienza el stream desde cero.',
    ],
    correctIndex: 1,
  ...
  // ── microfrontend ─────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Garantiza que solo un micro-frontend puede estar renderizando en la página al mismo tiempo, bloqueando el acceso al DOM para el resto mientras el primero está activo; los demás micro-frontends se encolan y se montan secuencialmente cuando el activo desmonta.',
      ...
      'Comparte el store de Redux del host automáticamente con todos los micro-frontends remotos sin necesidad de configuración adicional, permitiendo que cualquier micro-frontend acceda al estado global y despache acciones a los reducers registrados en el host.',
      'Evita que Module Federation descargue el bundle del remote más de una vez durante la sesión del usuario, almacenando el módulo en una caché del navegador con la URL del remoteEntry.js como clave, reduciendo así el consumo de ancho de banda en navegaciones repetidas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Son más rápidos en renderizado que los componentes React o Angular porque no tienen el overhead del Virtual DOM ni del sistema de Change Detection; el navegador los procesa directamente como elementos HTML nativos optimizados por el motor del DOM del navegador.',
      ...
      'Permiten compartir el estado completo de la aplicación entre micro-frontends directamente a través del Shadow DOM sin necesidad de postMessage ni de un bus de eventos, ya que el estado se sincroniza automáticamente entre todos los componentes que extienden del mismo Custom Element base.',
      'Web Components solo funcionan con frameworks que tienen soporte nativo para la Shadow DOM API, lo que excluye a React hasta la versión 19 y Angular hasta la versión 17; proyectos con versiones anteriores deben usar iframes como alternativa para la comunicación entre micro-frontends.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'El estado compartido ocupa demasiado espacio en memoria porque cada micro-frontend mantiene su propia copia en el heap de V8 aunque apunte al mismo módulo, resultando en un consumo proporcional al número de micro-frontends activos y no en un consumo único compartido.',
      ...
      'Solo se pueden compartir valores de estado primitivos como strings y números a través de módulos JS; los objetos y arrays compartidos de esta forma siempre se copian por valor, haciendo imposible la sincronización reactiva de estructuras de datos complejas entre micro-frontends.',
      'El estado compartido a través de módulos JS no persiste entre sesiones del navegador porque los módulos ES solo existen en memoria durante la sesión activa; para persistencia cross-session hay que combinarlo obligatoriamente con localStorage o cookies de sesión compartidas.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Importando directamente el store de Redux del shell usando Module Federation: el shell expone su store como módulo compartido con `exposes: { "./store": "./src/store" }` y las micro-apps lo importan dinámicamente para acceder al estado global y despachar acciones sin tener que duplicar el store.',
      ...
      'Compartiendo el Context de React entre el shell y el micro-frontend mediante Module Federation con `singleton: true` en el shared de React; al haber una única instancia de React ambas apps comparten el mismo árbol de contextos y el micro-frontend puede leer el contexto de autenticación del shell directamente.',
      'No es posible que un micro-frontend se comunique con el shell porque están compilados independientemente y ejecutados en contextos de módulo ES separados; cualquier intercambio de datos requiere forzosamente una API REST o GraphQL como intermediario entre los dos contextos.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Un servidor de orquestación dedicado que recibe todas las peticiones HTTP de los usuarios y las distribuye entre los diferentes micro-frontends según la ruta, similar a un API Gateway pero para el tráfico de frontend; cada micro-frontend tiene su propio servidor Express independiente detrás del shell.',
      ...
      'Una librería de componentes de UI compartida (Design System) que todos los micro-frontends importan como dependencia, garantizando la consistencia visual entre equipos sin que ningún micro-frontend tenga que implementar sus propios componentes base como botones, inputs o modales.',
      'Un proceso de build en CI/CD que combina todos los bundles de los micro-frontends en un único fichero JavaScript optimizado que se despliega como una SPA tradicional, aprovechando Module Federation solo para facilitar el desarrollo paralelo de los equipos.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Una API interna de Webpack que permite hacer broadcast de chunks de JavaScript a múltiples Web Workers simultáneamente durante la inicialización de Module Federation, optimizando la carga paralela de los remotes al distribuir el trabajo de parsing entre los workers disponibles.',
      ...
      'Es el mecanismo interno de Module Federation para sincronizar la versión y el estado de los shared modules entre host y remotes: Webpack lo usa internamente para detectar si un shared module como React ya está cargado en el host antes de descargar la versión del remote.',
      'BroadcastChannel funciona exclusivamente en el contexto de Service Workers y no puede usarse en la ventana principal ni en iframes; para comunicación entre tabs en la ventana principal hay que usar la API `localStorage` con el evento `storage` como alternativa compatible.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Los micro-frontends no pueden usar TypeScript porque Module Federation no soporta la generación de declaraciones de tipos entre bundles de diferentes equipos; cada micro-frontend debe exponer sus interfaces como ficheros JSON de schema que los consumidores convierten a tipos manualmente.',
      ...
      'Los micro-frontends siempre tienen mayor latencia que un monolito porque cada ruta requiere una petición de red adicional para descargar el bundle del remote correspondiente, lo que hace que la navegación entre secciones sea perceptiblemente más lenta que en una SPA tradicional bien optimizada.',
      'La consistencia visual no es posible en micro-frontends sin copiar el código CSS completo del Design System en cada micro-frontend porque Module Federation no comparte ficheros CSS estáticos, solo módulos JavaScript; los estilos deben duplicarse en cada equipo o usar CSS-in-JS para poder compartirlos.',
    ],
    correctIndex: 1,
  ...
  // ── spa ───────────────────────────────────────────────────────────────────
  {
    ...
    options: [
      'Los buscadores modernos no pueden indexar páginas que usen JavaScript porque sus crawlers no ejecutan JS por razones de seguridad; esto es un problema conocido de toda la web moderna y no es específico de CSR sino de cualquier página que dependa de JavaScript para su renderizado inicial.',
      ...
      'CSR genera URLs con fragmentos de hash (`#/ruta`) que los motores de búsqueda no pueden procesar ni indexar, porque el protocolo HTTP no envía la parte del hash al servidor y los crawlers no pueden distinguir diferentes páginas que comparten el mismo path pero tienen hashes distintos.',
      'CSR solo es problemático para el SEO en aplicaciones React porque Virtual DOM genera HTML con atributos data-react que los crawlers no interpretan correctamente; Angular Universal resuelve el mismo problema con CSR usando Server-Side Rendering transparente para los bots.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Es un error de tipado en los props del componente que TypeScript detecta en tiempo de compilación pero que solo se manifiesta en runtime cuando el servidor y el cliente usan versiones distintas del tipo, generalmente tras una actualización parcial del bundle sin invalidar la caché del CDN.',
      ...
      'Es cuando React intenta adjuntar los event listeners antes de que el bundle JS haya terminado de descargarse por completo, causando que los primeros clicks del usuario no respondan porque los handlers no están registrados aún en los elementos del DOM pre-renderizado.',
      'Solo afecta a aplicaciones React porque usa un algoritmo de comparación DOM-virtual que es sensible al orden de los nodos; Angular Universal usa un mecanismo de transferstate diferente que serializa el estado en el HTML y lo deserializa en el cliente sin comparación directa del DOM.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'SSG genera el HTML en el servidor para cada request del usuario igual que SSR, pero lo cachea internamente durante exactamente 1 hora por defecto; ISR añade la capacidad de configurar ese TTL con la opción `revalidate` para acortarlo o alargarlo según las necesidades del contenido.',
      ...
      'ISR está diseñado exclusivamente para rutas dinámicas con parámetros como `/blog/[slug]` donde el contenido cambia por ruta; para páginas completamente estáticas como `/about` o `/contact` solo se puede usar SSG porque ISR necesita saber qué parámetros pueden cambiar para gestionar la revalidación.',
      'SSG necesita un servidor Node.js en producción para servir los ficheros HTML porque el proceso de generación estática de Next.js produce ficheros que requieren el runtime de Next.js para procesarse; ISR es la única estrategia que genera HTML puro servible directamente desde una CDN sin servidor.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Hace prefetch del componente cuando el usuario pasa el cursor por encima del elemento que lo activaría, descargando el chunk JS en segundo plano para que esté disponible inmediatamente cuando el usuario navegue; sin `Suspense`, el prefetch no funciona correctamente porque React no puede gestionar el estado de carga pendiente.',
      ...
      'Es completamente equivalente a `import()` nativo de JavaScript pero con mejor manejo de errores y reintentos automáticos; la única diferencia técnica es que `React.lazy` registra el componente en el sistema de reconciliación de React para que los errores de carga aparezcan en el Error Boundary más cercano.',
      '`React.lazy` ejecuta el proceso de renderizado del componente en un Web Worker separado del hilo principal de React para evitar que los componentes pesados bloqueen el hilo de animación a 60fps; `Suspense` es el mecanismo de sincronización que espera a que el Worker termine antes de actualizar el DOM.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'Para redirigir automáticamente a los usuarios al login cuando intentan acceder directamente a rutas privadas como `/dashboard`; sin esta regla de fallback, el servidor no tendría forma de saber qué rutas requieren autenticación antes de que el JavaScript de la SPA se ejecute.',
  ...
  },
  {
    ...
    options: [
      'Añade estilos CSS automáticos al enlace para indicar visualmente que es una ruta interna de Next.js: muestra un color diferente, aplica la clase `active` cuando la ruta coincide con la URL actual, y añade un cursor de carga mientras el chunk de la página destino se está descargando.',
      ...
      '`<Link>` de Next.js es funcionalmente idéntico a `<a>` en términos de navegación pero añade soporte completo para TypeScript en las props `href` y `as`, con tipos genéricos que validan en tiempo de compilación que la ruta destino existe en el proyecto sin necesidad de configuración adicional.',
      '`<Link>` solo funciona correctamente en páginas con SSR porque necesita el runtime de Next.js en el servidor para precalcular las rutas disponibles; en páginas estáticas generadas con SSG, el componente se comporta como un `<a>` normal sin ningún tipo de prefetch ni gestión de estado del router.',
    ],
    correctIndex: 1,
  ...
  },
  {
    ...
    options: [
      'TTI mide el tiempo hasta que aparece el primer pixel de contenido en pantalla; code splitting lo mejora directamente porque al reducir el número de módulos que el navegador carga inicialmente, el First Contentful Paint ocurre antes sin esperar a que todos los chunks estén disponibles.',
      ...
      'TTI mide el tiempo que tarda el servidor en generar la primera respuesta HTTP con el HTML de la página; code splitting no lo mejora porque afecta solo al tiempo de procesamiento en el cliente, no a la velocidad de respuesta del servidor ni al tamaño del HTML transferido inicialmente.',
      'TTI es una métrica específica de Google para aplicaciones móviles medida solo en dispositivos Android; para aplicaciones de escritorio existe la métrica TTD (Time to Desktop) que usa diferentes umbrales y no se ve afectada por el code splitting en la misma medida.',
    ],
    correctIndex: 1,
