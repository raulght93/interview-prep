...
export const CHEATSHEETS: CheatSheet[] = [
  ...
  },
  {
    topicId: 'redux',
    points: [
      'Flujo unidireccional: UI → dispatch(action) → reducer (fn pura) → nuevo estado → re-render.',
      'createSlice: genera actions + reducer juntos. Immer permite "mutar" el draft (es inmutable internamente).',
      'createAsyncThunk: genera 3 actions (pending/fulfilled/rejected). extraReducers para manejarlas en el slice.',
      'createSelector (Reselect): memoiza selectores derivados. Solo recalcula si cambian los inputs.',
      'configureStore: junta slices, añade middleware (thunk por defecto) y conecta DevTools automáticamente.',
      'RTK Query: endpoints + hooks autogenerados (useGetXQuery, useCreateXMutation). Cache, invalidación, refetch.',
      'Redux vs Context: usa Redux para estado global frecuente/complejo. Context para config raramente actualizada.',
      'DevTools Extension: time-travel debugging, inspección de actions y estado en cada paso.',
      'Regla: los reducers son SIEMPRE funciones puras (sin I/O, sin Date.now(), sin side effects).',
    ],
  },
  {
    topicId: 'ngrx',
    points: [
      'Flujo: Component → dispatch(action) → reducer → Store → selector → Component. Effects para side effects.',
      'createAction + props<{...}>(): tipado automático del payload. Convención: "[Feature] Event Name".',
      'createReducer + on(): reemplaza el switch. Immer integrado para "mutar" el estado del draft.',
      'createEffect: observable de actions. ofType() filtra. switchMap/mergeMap para I/O. Devuelve nueva action.',
      'createSelector: memoización con Reselect. Componentes con OnPush: solo se renderizan si el selector emite.',
      'Entity Adapter: addOne/updateOne/removeMany/selectAll. Mejor que arrays para colecciones por ID.',
      'Signal Store (v17+): signalStore + withState/withComputed/withMethods/withHooks. Sin Actions ni Effects.',
      'Store.dispatch() en el componente. Store.select(selector$) devuelve Observable. async pipe en template.',
      'Efecto de dispatch:false para side effects sin acción de resultado: { dispatch: false } en createEffect.',
    ],
  },
  {
    topicId: 'rxjs',
    points: [
      'Observable: lazy, cold por defecto (nueva ejecución por suscriptor). Subject: hot, emite a todos.',
      'BehaviorSubject(init): emite el último valor al suscribirse. Ideal para estado. ReplaySubject(n): últimos N.',
      'switchMap: cancela anterior (typeahead). mergeMap: paralelo (independientes). concatMap: orden estricto. exhaustMap: ignora mientras activo (submit).',
      'combineLatest: emite con último de todos cuando cualquiera emite. forkJoin: todos completan → un array.',
      'takeUntil(destroy$) o takeUntilDestroyed(): cancelar al destruir. async pipe: Angular gestiona la suscripción.',
      'shareReplay(1): convierte a hot + cachea último valor. Para HTTP compartido entre varios consumidores.',
      'catchError(err => of(fallback)): recuperar el stream. throwError(() => err): re-lanzar. retry(n): reintentar.',
      'debounceTime(300): espera silencio antes de emitir. distinctUntilChanged: filtra duplicados consecutivos.',
      'tap: side effect sin transformar. map: síncrono 1→1. filter: descarta valores. scan: reduce acumulativo.',
    ],
  },
  {
    topicId: 'storage',
  ...
  },
  {
    ...
    points: [
      ...
      'shared: { react: { singleton: true } }: una sola instancia. Sin esto, los hooks fallan.',
      'Web Components: Custom Elements como interfaz estándar entre micro-apps. @r2wc/react-to-web-component.',
      'DynamicWebComponent (Agata): triple caché React state → blob URL → network. Latencia cero tras primer load.',
      'WidgetMessenger/postMessage: protocolo tipado (MessageTypes) para comunicación bidireccional host↔widget.',
      'iframe: máximo aislamiento (CSS, JS, errores). Comunicación solo via postMessage. Limita UX (resize, links).',
      'BroadcastChannel: comunicación entre tabs/iframes del mismo origen. Sin servidor intermedio.',
      'Trade-off: autonomía de equipos a cambio de complejidad (shared deps, debugging cross-app, bundle weight).',
      'Shell pattern: SPA raíz que gestiona auth + layout global y carga micro-apps según la ruta activa.',
    ],
  },
  {
    topicId: 'spa',
    points: [
      'CSR: HTML vacío + JS → mal FCP, buena DX. SSR: HTML del servidor → buen SEO, carga de servidor.',
      'SSG: build-time → estático en CDN → velocidad máxima. ISR: SSG + revalidación en background cada N seg.',
      'Hidratación: JS en cliente adjunta event listeners al HTML pre-renderizado. Mismatch → error de hydration.',
      'Code splitting: lazy(import("./Page")) + Suspense. Next.js lo hace por página automáticamente.',
      'History API: pushState/replaceState actualiza la URL sin recarga. El router intercepta clicks en <a>.',
      'Prefetch: <Link> de Next.js pre-descarga la ruta al hacer hover. Angular Router: preloadingStrategy.',
      'Problema CSR + SEO: los bots ven HTML vacío si no espera la ejecución JS. SSR/SSG lo resuelven.',
      'Shell pattern: la SPA carga micro-apps por ruta. Gestiona auth y layout; las micro-apps su dominio.',
      'TTI (Time to Interactive): tiempo hasta que la app es totalmente interactiva. Reducir con code splitting.',
    ],
  },
];
